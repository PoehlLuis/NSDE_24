...
class EXPORT:
    def __init__(self, nodesPerEl, nElements, nNodes, dim, U, geom, connec_plot,
                 n_dof_p_node):  # "self" will be replaces by the name of the object (e.g. here "domain")
        ''' 
        ...
        self.connec_plot = self.connec_plot + np.ones((self.nElements,
                                                       nodesPerEl))  # adds +1 to every element in the matrix (changes indexing from 0,1,... to 1,2,...)
    ...
    def writeResults(self):
        ...
        # =====================================================================================================
        # MESH/ELEMENTS
        dsetConnectivity = grpElements.create_dataset("Connectivity", (self.nElements, self.nodesPerEl),
                                                      dtype='i')  # creates empty connectivity matrix (nElements, nodesPerEl)
        dsetConnectivity[...] = self.connec_plot
        dsetTypes = grpElements.create_dataset("Types", (self.nElements,),
                                               dtype='i')  # creates empty el. types matrix (nElements, )
        if self.nodesPerEl == 4:
            ...
            # MESH/NODES
        dsetCoordinates = grpNodes.create_dataset("Coordinates", (
        self.nNodes, 3))  # creates node coordinates matrix (nNodes, dimension)
        ...
        dt = h5py.special_dtype(
            vlen=str)  # h5py.string_dtype(encoding='utf-8')                                              # sets variableType = string with variable lenght
        if (self.isScalarField):
